...
class TC_Library < Test::Unit::TestCase
  ...
  def test_search_multicopy_all_available
    @lib.open
    res = @lib.search('test book')
    assert res == @lib.books[198].to_s, "Returned: #{res}"
  end

  def test_search_multicopy_none_available
    @lib.open
    @lib.issue_card('bob')
    @lib.serve('bob')
    @lib.check_out(199, 201)
    res = @lib.search('test book')
    assert res == 'No books found.', "Returned: #{res}"
  end

  def test_search_multiauthor_all_available
    @lib.open
  ...
  def test_search_multiauthor_some_available
    @lib.open
    @lib.issue_card('bob')
    @lib.serve('bob')
    @lib.check_out(80)
